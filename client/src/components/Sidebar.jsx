...
const adminLinks = [
  ...
  { to: '/admin-leave-management', icon: 'fa-calendar', label: 'Leave Management' },
  { to: '/attendance', icon: 'fa-calendar-check', label: 'Attendance' }, // NEW: mark/manage attendance
...
function employeeLinks(isManager) {
  const base = [
    { to: '/dashboard', icon: 'fa-tachometer-alt', label: 'Dashboard' },
    { to: '/wallet', icon: 'fa-building', label: 'HR Wallet' },
    { to: '/payslip', icon: 'fa-file-invoice-dollar', label: 'My Payslips' },
  ];
  // Manager status comes from the `managers` table via user.isManager
  // (set by the backend on login/me) — NOT from user.role, since role
  // for employees is just a numeric role_id (their job title).
  if (isManager) {
    base.push({ to: '/head-leave-management', icon: 'fa-calendar', label: 'Leave Management' });
  }
  return base;
}
...
export default function Sidebar() {
  ...
  // employeeLinks(...) is CALLED here — it returns an array, which is
  // what .map() below needs. Assigning the function itself (without the
  // parentheses) is what caused "links.map is not a function".
